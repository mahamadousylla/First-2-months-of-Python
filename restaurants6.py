print('Mahamadou Sylla ID: 61549479 ICS 31 sec 8 Lab Asst 6')
...
Restaurant Collection Program --- Choose one
 ...
 g:  Search for all restaurants that serve this dish
 f:  Find restaurants at or below certain price
 l:  Enter a dish phrase to search for restaurants that serve that dish
 q:  Quit
"""
...
def handle_commands(C: list) -> list:
    ...
    while True:
        ...
        if response=="q":
            ...
            name = str(input("Enter type of cuisine you are looking for: "))
            print(Collection_str(Collection_search_by_cuisine(C, name)))
           
            lisst = []
            for restaurant in Collection_search_by_cuisine(C, name):
                lisst.extend(restaurant.menu)
            print('{}: ${}.'.format('Average price', str(Dishlist_average(lisst))))
        elif response == 'l':
            phrase = input('Enter a dish phrase to search for: ')
            print(Collection_search_by_phrase(C, phrase))
        else:
...
def Restaurant_str(self: Restaurant) -> str:
    return (
        ...
        '{}: ${:.2f}. {}: {:.2f} '.format('Average price', Dishlist_average(self.menu), 'Average calories', Dishlist_average_calories(self.menu))+ '\n\n')
...
def Collection_search_by_cuisine(C, name):
    """ Return list of Restaurants in input list whose name matches input string.
    """ 
    x=[]
    for restaurant in C:
        if restaurant.cuisine == name:
            x.append(restaurant)
    return x
...
def Collection_search_by_phrase(C, phrase):
    '''allows the user to search for (and display)
    all the restaurants that serve a dish whose name
...
def Dishlist_calories(list_of_dishes):
    '''take in a list of dishes and
    returns a list of numbers containing
    just the calories of the dishes'''
    x=[]
    for dish in list_of_dishes:
        x.append(dish.calories)
    return x


def Dishlist_average_calories(list_of_dishess):
    '''takes in a list of dishes and
    returns the average calorie count of those dishes'''
    return (sum(Dishlist_calories(list_of_dishess)) / len(Dishlist_calories(list_of_dishess)))
