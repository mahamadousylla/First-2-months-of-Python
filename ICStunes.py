...
def Song_str(song):
    return '{}---{}---{}---{}'.format(song.track, song.title, song.length, song.play_count)
...
def Songdisplay_str(songdisplay):
    x=''
    for item in songdisplay:
        x+='{}---{}---{}---{}---{}---{}---{}'.format(item.artist, item.a_title, item.year, item.track, item.s_title, item.length, item.play_count)+ '\n'
    return x
print(Songdisplay_str(top_n_played(MUSIC, 10)))
print()
...
def album_play(album):
    return len(album.artist)
MUSIC.sort(key=album_play)
a=''
for album in MUSIC:
    a+= Album_str(album) + '\n\n'
print(a)
...
def length_from_songdisplay(songdisplay):
    return songdisplay.length
print(length_from_songdisplay(Songdisplay(artist='The Rolling Stones', a_title='Let It Bleed', year=1969, track=3, s_title='Country Honk', length=187, play_count=0)))

print('-------------------------------------e.5-----------------------------------')
print()

def favorite_album(collection):    
    for album in collection:
        count = 0
        localCount=0
        albumName=''
        for song in album.songs:
            localCount+=(song.length * song.play_count)
##        print(localCount)
        if localCount > count:
            count = localCount
            albumName = album
    return Album_str(albumName)
print(favorite_album(MUSIC))
        
print('-------------------------------------e.6-----------------------------------')
print()

def top_n(collection, integer, sort_key, boolean):
    list_of_Songdisplays = all_Songdisplays(collection)
    list_of_Songdisplays.sort(key=sort_key, reverse=boolean)
    return list_of_Songdisplays[:integer]
##print(top_n_played(MUSIC, 3))
print()
print('Sorted from highest to lowest:')
print()
print(Songdisplay_str(top_n(MUSIC, 3, play_count_from_songdisplay, True)))
print()
print('Sorted from lowest to highest:')
print()
print(Songdisplay_str(top_n(MUSIC, 10, length_from_songdisplay, False)))
    
print('-------------------------------------e.7-----------------------------------')
print()    

def Album_listening_time(album):
    count=0
    for song in album.songs:
        count+=song.length
    return count

def favorite_album2(collection, keyFunction):
    collection.sort(key=keyFunction)
    return collection[-1]
...
def collection_search(collection, string):
    result= [ ]
    for album in collection:
        for song in album.songs:
            if string in album.title:
                result.append(Album_to_Songdisplays(album))
            if string in album.artist:
                result.append(Album_to_Songdisplays(album))
            if string in song.title:
                result.append(Songdisplay(album.artist, album.title, album.year, song.track, song.title, song.length, song.play_count))
    result = list(set(result))
    return result
print(Songdisplay_str(collection_search(MUSIC, 'ou')))        
